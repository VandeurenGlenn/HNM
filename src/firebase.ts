...
export const get = async (path: string): Promise<FirebaseDatabaseFormat> => {
  const snap = await _get(ref(database, path))
  return snap.val()
}

export const push = async (path: string, value: FirebaseDatabaseFormat): Promise<string> => {
...
export const update = async (path: string, value: any): Promise<void> => _update(ref(database, path), value)

export const createUserWithEmailAndPassword = async (email, password): Promise<UserCredential> =>
